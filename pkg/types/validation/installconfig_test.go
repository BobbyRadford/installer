...
func validIBMCloudPlatform() *ibmcloud.Platform {
	return &ibmcloud.Platform{
		Region:         "us-south",
		CISInstanceCRN: "crn:v1:bluemix:public:internet-svcs:us-south:a/account:instance::",
...
func TestValidateInstallConfig(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			expectedError: `^\[platform\.ibmcloud\.region: Required value: region must be specified, platform\.ibmcloud\.clusterOSImage: Required value: clusterOSImage must be specified, platform\.ibmcloud\.cisInstanceCRN: Required value: cisInstanceCRN must be specified\]$`,
		},
		{
