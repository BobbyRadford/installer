...
func Validate(client API, ic *types.InstallConfig) error {
	...
	allErrs = append(allErrs, validateResourceGroups(client, ic, field.NewPath("platform").Child("ibmcloud"))...)
	allErrs = append(allErrs, validateNetworks(client, ic, field.NewPath("platform").Child("ibmcloud"))...)
...
func validateInstanceTypes(client API, ic *types.InstallConfig) field.ErrorList {
	...
	zones, err := client.GetZones(context.TODO(), ic.IBMCloud.ProjectID, fmt.Sprintf("region eq .*%s", ic.IBMCloud.Region))
	if err != nil {
		return append(allErrs, field.InternalError(nil, err))
	}
	...
	if ic.ControlPlane != nil && ic.ControlPlane.Platform.IBMCloud != nil && ic.ControlPlane.Platform.IBMCloud.InstanceType != "" {
		// Default requirements can be relaxed when the controlPlane type is set explicitly.
		defaultInstanceReq = computeReq

		allErrs = append(allErrs, ValidateInstanceType(client, field.NewPath("controlPlane", "platform", "ibmcloud"), ic.IBMCloud.ProjectID, zones[0].Name,
			ic.ControlPlane.Platform.IBMCloud.InstanceType, controlPlaneReq)...)
	}

	if ic.Platform.IBMCloud.DefaultMachinePlatform != nil && ic.Platform.IBMCloud.DefaultMachinePlatform.InstanceType != "" {
		allErrs = append(allErrs, ValidateInstanceType(client, field.NewPath("platform", "ibmcloud", "defaultMachinePlatform"), ic.IBMCloud.ProjectID, zones[0].Name,
			ic.Platform.IBMCloud.DefaultMachinePlatform.InstanceType, defaultInstanceReq)...)
	}

	for idx, compute := range ic.Compute {
		fieldPath := field.NewPath("compute").Index(idx)
		if compute.Platform.IBMCloud != nil && compute.Platform.IBMCloud.InstanceType != "" {
			allErrs = append(allErrs, ValidateInstanceType(client, fieldPath.Child("platform", "ibmcloud"), ic.IBMCloud.ProjectID, zones[0].Name,
				compute.Platform.IBMCloud.InstanceType, computeReq)...)
		}
	}
...
func ValidatePreExitingPublicDNS(client API, ic *types.InstallConfig) error {
	...
	zone, err := client.GetPublicDNSZone(context.TODO(), ic.Platform.IBMCloud.ProjectID, ic.BaseDomain)
	if err != nil {
		var gErr *googleapi.Error
		if errors.As(err, &gErr) {
			if gErr.Code == http.StatusNotFound {
				return field.NotFound(field.NewPath("baseDomain"), fmt.Sprintf("DNS Zone (%s/%s)", ic.Platform.IBMCloud.ProjectID, ic.BaseDomain))
			}
		}
		return field.InternalError(field.NewPath("baseDomain"), err)
	}

	rrSets, err := client.GetRecordSets(context.TODO(), ic.Platform.IBMCloud.ProjectID, zone.Name)
	if err != nil {
		return field.InternalError(field.NewPath("baseDomain"), err)
	}
	...
	for _, r := range rrSets {
		if strings.EqualFold(r.Name, record) {
			return field.Invalid(field.NewPath("metadata", "name"), ic.ObjectMeta.Name, fmt.Sprintf("record %s already exists in DNS Zone (%s/%s) and might be in use by another cluster, please remove it to continue", record, ic.Platform.IBMCloud.ProjectID, zone.Name))
		}
	}
	return nil
}

func validateResourceGroups(client API, ic *types.InstallConfig, fieldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}

	if ic.IBMCloud.ProjectID != "" {
		...
		if _, found := resourceGroups[ic.IBMCloud.ProjectID]; !found {
			return append(allErrs, field.Invalid(fieldPath.Child("project"), ic.IBMCloud.ProjectID, "invalid project ID"))
		}
	}
...
func validateNetworks(client API, ic *types.InstallConfig, fieldPath *field.Path) field.ErrorList {
	...
	if ic.IBMCloud.Network != "" {
		_, err := client.GetNetwork(context.TODO(), ic.IBMCloud.Network, ic.IBMCloud.ProjectID)
		if err != nil {
			return append(allErrs, field.Invalid(fieldPath.Child("network"), ic.IBMCloud.Network, err.Error()))
		}

		subnets, err := client.GetSubnetworks(context.TODO(), ic.IBMCloud.Network, ic.IBMCloud.ProjectID, ic.IBMCloud.Region)
		if err != nil {
			return append(allErrs, field.Invalid(fieldPath.Child("network"), ic.IBMCloud.Network, "failed to retrieve subnets"))
		}

		allErrs = append(allErrs, validateSubnet(client, ic, fieldPath.Child("computeSubnet"), subnets, ic.IBMCloud.ComputeSubnet)...)
		allErrs = append(allErrs, validateSubnet(client, ic, fieldPath.Child("controlPlaneSubnet"), subnets, ic.IBMCloud.ControlPlaneSubnet)...)
	}
...
func validateSubnet(client API, ic *types.InstallConfig, fieldPath *field.Path, subnets []*compute.Subnetwork, name string) field.ErrorList {
	...
	subnet, errMsg := findSubnet(subnets, name, ic.IBMCloud.Network, ic.IBMCloud.Region)
	if subnet == nil {
